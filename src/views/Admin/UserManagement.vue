<template>
  ...
  <a-table :columns="columns" :data-source="dataSource" bordered>
    <template #bodyCell="{ column, text, record }">
      <template v-if="['username', 'password', 'nickname', 'email', 'phone', 'address', 'avatarUrl'].includes(column.dataIndex)">
        <div>
          <a-input
...
const onDelete = async (key) => {
  ...
  console.log(item.id);
  // 请求后端删除数据
  const res = await myAxios.post(`/user/delete/${item.id}`);
  if (res.code === 0) {
...
const handleOk = async () => {
  // 请求后端，添加表格项
  const res = await myAxios.post('/user/add', formModal.value);
  if (res.code == 0) {
    message.success('添加成功');
    open.value = false;
  } else {
    message.error('添加失败' + (res.message ? `，${res.message}` : ''));;
  }
  // 重新加载表格数据
  loadData();
};
